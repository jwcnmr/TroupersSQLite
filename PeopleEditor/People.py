...
class TMediator():
    ...
    def moveLeft(self):
        ...
        print(patkey,officekey)
        #qry="select frname, lname from patrons, offices.officename where personkey="+str(patkey)+" and offices.officekey="+str(officekey)
        qry="select frname, lname , officename from patrons, offices where patrons.personkey="+str(patkey)+" and offices.officekey="+str(officekey)
        print (qry)
        query =Query(self.db.cursor, qry)
        results = query.execute()
        rows = results.getRows()
        ...
        for r in rows:
            names+= str(r)+" "
        qry = "Insert into board (personkey, officekey) values(%s,%s)"
        val = (patkey, officekey)
    ...
    def fillList(self):
       ...
       qry="""select frname, lname , officename from patrons, offices, board 
       where patrons.personkey=board.personkey and offices.officekey=board.officekey 
       order by offices.officekey, lname"""
       query=Query(self.db.cursor, qry)
       results = query.execute()
       rows = results.getRows()
       for row in rows:
...
class TableDisplay():
    def __init__(self, med):
        ...
        self.frame.title("Board of Directors")
        self.mainTree = Treeview(self.frame)
        self.mainTree = Treeview(self.frame, height=14)
        #self.mainTree.grid(row=0, column=0)

        self.mainTree["columns"] = ("frname", "lname", "office","address", "town", "state", "zip", "email", "phone", "phone2")
        ...
        qry = """select patrons.personkey, frname, lname, officename, address, town, state, zipcode, email, phone, phone2 from patrons, offices, board 
              where patrons.personkey=board.personkey and offices.officekey=board.officekey 
              order by offices.officekey, lname"""
...
class ExportButton(DButton):
    ...
    def comd(self):
        ...
        CsvExport.export(self.results, "Board.csv", self.med.getColnames())

# static class takes a resultSet and writes a csv file
# if an element contains a comma, it is enclosed in quotes
class CsvExport():
    @staticmethod
    def export(results, initialName, headers ):
        fname = filedialog.asksaveasfilename(initialfile=initialName)
        if len(fname)>0:
            with open(fname, "w") as f:
                ...
                for r in rows:
                    ...
                    for s in r:
                        s1 = str(s)
                        if "," in s1:  # surround with quotes if it contains a comma
                             s1 = '\"' + s1 + '\"'
                        cline = cline + str(s1) + ","
                    f.write(cline + "\n")
...
class BoardEditor():
    ...
    def createBoard(self, db):
        qry = "create table Board (boardKey int auto_increment primary key, personkey int not null, officekey int not null)"
        print(qry)
        query = Query(db.cursor, qry)
        query.execute()
        qry = "create table Offices (officeKey int auto_increment primary key, officename varchar(40))"
        print(qry)
        query = Query(db.cursor, qry)
        query.execute()
        qry = "insert into Offices (officename) values (%s)"
        val = ("President")
    ...
    def showBoard(self):
        ...
        lframe = LabelFrame(self.frame,text="Officers")
        ...
        groupv = tk.IntVar()
        groupv.set(5)  # initializing the choice
        ChoiceButton(lframe, 'President', 1, groupv)
        ChoiceButton(lframe, 'Vice President', 2, groupv)
        ChoiceButton(lframe, 'Secretary', 4, groupv)
        ChoiceButton(lframe, 'Treasurer', 3, groupv)
        ChoiceButton(lframe, 'member', 5, groupv)
        self.med.setChoiceArg(groupv)
