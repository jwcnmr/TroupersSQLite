...
class TMediator():
    ...
    def moveLeft(self):
       ...
       # officekey = self.groupv.get()
        ...
        for r in rows:
            names += str(r) + " "
        qry = "Insert into lifers (personkey) values("+str(patkey)+")"
        query = Query(self.db.cursor, qry)
        query.execute()
        self.db.commit()
    ...
    def fillList(self):
        ...
        qry = """select frname, lname from patrons, lifers 
       where patrons.personkey=lifers.personkey 
       order by  lname"""
        query = Query(self.db.cursor, qry)
        results = query.execute()
        rows = results.getRows()
...
class TableDisplay():
    def __init__(self, med):
        ...
        qry = """select patrons.personkey, frname, lname, address, town, state, zipcode, email, phone, phone2 from patrons, lifers 
              where patrons.personkey=lifers.personkey 
              order by lname"""
...
class ExportButton(DButton):
    ...
    def comd(self):
        ...
        CsvExport.export(self.results, "Lifers.csv", self.med.getColnames())
