""" This is a copy of Donations.p frp, tje Donaiton module
modified to read the nicknames from a donor csv file and store the
nicknames in the patrons table"""
    ...
    def getFullQUery(self):
        qry = """select donorkey, frname, lname from patrons, donors
                      where patrons.personkey=donors.personkey
                          order by  lname"""
        qry = """select patrons.personkey, frname, lname from patrons, donors
                              where patrons.personkey=donors.personkey
                                  order by  lname"""
...
# adds a donation manually from date and amount fields using key from donors column
    ...
    def nameClick(self, evt):
        if not self.adding:
            ...
            i = int(index[0])
            key = self.keys[i]
            qry="""select distinct nickname, lname from patrons, donations, 
                donors where donors.donorkey=donations.donorkey and patrons.personkey=donors.personkey
                and patrons.personkey="""+str(key) +" order by date desc"
            qry = """select nickname, lname from patrons where patrons.personkey="""+str(key)
            print (qry)
            query=Query(self.db.cursor, qry)
            results = query.execute()
            rows = results.getDictRows()
            if len(rows) >0:
                r = rows[0]
                nm = r.get('nickname')
                self.fillNick(nm)
    ...
    def setNick(self, nick):
        self.nickfield = nick
    def fillNick(self, nickstr):
        self.nickfield.delete(0, END)
        self.nickfield.insert(0, nickstr)
    def getAdding(self):
        return self.adding
    def setDate(self, dt):
        self.date=dt
    ...
    def fillDonors(self, donors):
        ...
        for d in self.donors:
            self.donorlist.insert(END, d.getName()+" "+d.getNickname())

    # donor clicked - fill date and amount and put name in search box
    def donClick(self, evt):
       ...
       # self.date.insert(0, donor.getDate())
        ...
        self.donation.insert(0, donor.getNickname())
...
# in this version it only adds the nickname
class AddNicknameButton(DButton):
    def __init__(self, master, med, **kwargs):
        self.med = med
        super().__init__(master, text="Add nickname", **kwargs)
...
#        self.med.setTreetop(self.frame)
    ...
    def sumqry(self, pdate):
        if len(pdate)<=0:
            var0="select patrons.personkey, frname, lname, sum(Amount), ANY_VALUE(donations.Date), address, town,"
...
# Donor class holds one imported donor, here just to store the nickname
...
class NicknameEditor():
    ...
    def showDonors(self):
        ...
        lb1=Label(box1,text="Nickname", foreground='blue')
        lb1.grid(row=0, column=1, sticky=N)
        self.nickname = Entry(box1, width=20)
        self.nickname.grid(row=1, column=1, sticky=N)
        self.med.setNick(self.nickname)
       ...
       # box2.grid(row=2, column=1, sticky=N)
        ...
        #self.med.setDonation(self.nickname)
        #self.med.setDate(self.date)

        mlb = AddNicknameButton(self.frame, self.med)
